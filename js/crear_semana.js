    ...
    /*fetch("http://localhost:3000/graphql", {
      ...
      body: JSON.stringify({
        query: `
          mutation {
            crearCard(input: {
              semana: "${cardData.semana}",
              anio: "${cardData.anio}",
              descripcion: "${cardData.descripcion}",
              mes: "${cardData.mes}",
              horas: "${cardData.horas}",
      ...
      .then((data) => {
        const nuevaCard = data.data.crearCard;
        console.log("Nueva card creada:", nuevaCard);*/
        ...
        let card = document.createElement("div");
        card.className = "card";
        let cardId = "card-" + nuevaCard.id;
        card.setAttribute("id", cardId);
        card.style.backgroundColor = nuevaCard.color;
        card.style.width = "320px";
        card.style.height = "250px";
        ...
        let cardTitle = document.createElement("h5");
        cardTitle.className = "card-title";
        cardTitle.textContent = "Semana " + nuevaCard.semana + " - " + nuevaCard.anio;
        let cardText = document.createElement("p");
        cardText.className = "card-text";
        cardText.style.maxHeight = "80px";
        cardText.style.overflow = "hidden";
        cardText.textContent = nuevaCard.descripcion;
        let cardFooter = document.createElement("div");
        cardFooter.className = "card-footer";
        let cardFooterText = document.createElement("small");
        cardFooterText.className = "text-muted";
        cardFooterText.textContent = nuevaCard.mes + " - " + nuevaCard.horas + " horas";
        let button1 = document.createElement("button");
        button1.className = "btn btn-primary btn-sm";
        button1.textContent = "Detalles";
        button1.style.backgroundColor = "#CFD7C7";
        ...
        button2.addEventListener("click", function () {
          ...
          eliminarBtn.addEventListener("click", function () {
            ...
            fetch("http://localhost:3000/graphql", {
              ...
              .then((response) => response.json())
              .then((data) => {
                console.log("Tarea eliminada:", data.data.deleteTarea);
              })
              .catch((error) => {
